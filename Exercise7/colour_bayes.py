...
def main(infile):
    ...
    # print("model training rgb score:\t", model_rgb.score(X_train, y_train))
    print("model validation rgb score:\t", model_rgb.score(X_valid, y_valid))

    # TODO: build model_lab to predict y from X by converting to LAB colour first.
    lab_colors = rgb2lab(X)
    model_lab = make_pipeline(FunctionTransformer(rgb2lab_transform, validate=True), GaussianNB())    
    model_lab.fit(X_train, y_train)
    
    # TODO: print model_lab's accuracy score
    print("model validation lab score:\t", model_lab.score(X_valid, y_valid))
