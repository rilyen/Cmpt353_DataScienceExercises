...
def rgb2lab_transform(X):
    X = X.reshape(-1,1,3) # reshape to use in rgb2lab, 3 channels
...
def main():
    ...
    bayes_convert_model = make_pipeline(FunctionTransformer(rgb2lab_transform, validate=True), GaussianNB())    
    knn_rgb_model = KNeighborsClassifier(n_neighbors=5)
    knn_convert_model = make_pipeline(FunctionTransformer(rgb2lab_transform, validate=True), KNeighborsClassifier(n_neighbors=5))
    rf_rgb_model = RandomForestClassifier(n_estimators=100, max_depth=3, min_samples_leaf=10)
    rf_convert_model = make_pipeline(FunctionTransformer(rgb2lab_transform, validate=True), RandomForestClassifier(n_estimators=100, max_depth=3, min_samples_leaf=10))
